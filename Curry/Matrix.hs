{-# LANGUAGE Haskell2010, FlexibleContexts, FlexibleInstances, TupleSections, TypeSynonymInstances, NoMonomorphismRestriction #-}
...
import Data.Array.Unboxed (IArray, UArray, (!), (//),
                   accum, array, assocs, bounds, elems, ixmap, amap,
                   listArray)
...
type Vector b = UArray Int b
type Matrix b = UArray (Int, Int) b
...
hilb :: Int -> Matrix Double
hilb n = listArray ((1, 1), (n, n)) [fromRational $ 1.0 / toRational (i + j - 1)
                                       | i <- [1 .. n],
                                         j <- [1 .. n]]
...
--liftOp :: (Num a, Num b, Ix i) => (a -> b -> a) -> UArray i a -> UArray i b -> UArray i a
liftOp o = withBoundsCheck (==) (\l r -> accum o l $ assocs r)

instance (IArray UArray b, Num b) => Num (Vector b) where
  (+) = liftOp (+)
  (*) = liftOp (*)
  abs = amap abs
  negate = amap negate
...
instance (IArray UArray b, Num b) => Num (Matrix b) where
  (+) = liftOp (+)
...
isSingular :: (Num b, Eq b, IArray UArray b) => Matrix b -> Bool
...
diagonal :: IArray UArray b => Matrix b -> Vector b
diagonal a
...
--maxLocBy :: (Ord e, Ix i) => (e -> e -> Ordering) -> UArray i e -> i
maxLocBy cmp = fst . maximumBy (cmp `on` snd) . assocs

--maxAbsLoc :: (Ord e, Num e, Ix i) => UArray i e -> i
